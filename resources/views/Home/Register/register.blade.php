					...
					<form action="/" method="post" id="ff">
						...
						<div class="form-group">
							<div class="input-group">
								...
								<input class="" name="phone" required placeholder="手机号" type="text" reminder="请输入正确的手机号" style="width: 260px;height: 40px;"><span></span>
							</div>
						</div>
						<div class="form-group">
							<div class="input-group">
...
<script>
	...
	$("input[name='repassword']").blur(function(){
		...
		o=$(this);
		//正则匹配 match 匹配不到的话,返回null
		if(cfm.match(/^[\w]{6,12}$/)==null){
			$(this).next("span").css('color','red').html('重复密码不一致');
			$(this).addClass('aa');
			CFMIMA=false;
		}else{
			//判断重复面是否一致
			$.get("/checkmima",{cfm:cfm},function(data){
				//alert(data);
				if(data==1){
					//重复密码不一致
					o.next("span").css('color','green').html('重复密码不一致');
					o.addClass('bb');
					CFMIMA=false;
				}else{
					//重复密码一致
